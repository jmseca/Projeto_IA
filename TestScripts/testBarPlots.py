...
if __name__ == "__main__":
    ...
    for problems in problems_results:
        df = pd.DataFrame(columns=["Category","N","Algoritmo"])
        #print(len(problems))
        for a in range(len(problems)):
            ...
            searchers_str[a]]
            df.loc[len(df.index)] = ["Nós Gerados",algo_result.succs,searchers_str[a]]
            df.loc[len(df.index)] = ["Nós Expandidos",algo_result.states,searchers_str[a]]
        #i+=1
    ...
    #    sb.barplot(data=dfs[i],x="Category",y='N',hue='Algorithm')
        sb.barplot(data=dfs[i],x="Category",y='N',hue='Algoritmo',ci=None)
        plt.legend(bbox_to_anchor=(1.02, 1), loc='upper left', borderaxespad=0)
        plt.savefig("Images/{}.png".format(files[i]), bbox_inches='tight')
